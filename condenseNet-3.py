...
def log(log_file_name, app_name="condenseNet-3"):
    """
...
def expand(input_tensor, input_dim, expand_depth, layer_name, num_1x1_filter, act=tf.nn.relu):
    ...
    with tf.name_scope(layer_name):
        ...
        pool_layer = max_pool(activations, filter_size=[1,4,4,1], strides=[1,4,4,1])
...
def model(x_image, keep_prob):
    ...
    # FC-1 layer
    fc_1 = add_fully_connected_layer(input_dim=4 * 4 * 128, output_dim=64, layer_name="fc1",
                                     keep_prob=keep_prob, input_tensor=condensed_layer)
    # FC-2 layer
    pred = add_fully_connected_layer(input_dim=64, output_dim=10, keep_prob=keep_prob, layer_name="fc2",
